...
export type Sect='diurna'|'nocturna';
export type Segment={start:number;end:number;ruler:string};
export const normalizeLongitude=(longitude:number)=>{if(!Number.isFinite(longitude))throw Error('Longitud no finita.');return ((longitude%360)+360)%360;};
export function intervalAt(segments:ReadonlyArray<Segment>,degree:number){const interval=segments.find(s=>degree>=s.start&&degree<s.end);if(!interval)throw Error('Posición fuera de los intervalos.');return interval;}
export function positionRules(longitude:number,sect:Sect){
 if(sect!=='diurna'&&sect!=='nocturna')throw Error('Falta la secta calculada respecto del horizonte.');
...
export function evaluateDignities(planet:string,longitude:number,sect:Sect){
 if(!['Sol','Luna','Mercurio','Venus','Marte','Júpiter','Saturno'].includes(planet))throw Error('Solo los siete planetas tradicionales tienen dignidades en este módulo.');
