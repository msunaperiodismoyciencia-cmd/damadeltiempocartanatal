...
import {renderAspectTable,compactAspects,compactCsv,type AspectOrder} from './aspect-ui.js';
...
import {renderWheel,defaultOptions,downloadSvg,downloadBlob,pngBlob,termLabel,type WheelData} from './wheel.js';
import {evaluateAspects,resetOrbs,medievalOrbs,traditionalPlanets,type Orbs} from './aspects.js';
type Body = {name:string;longitude:number;position:string;house:number;house_system:string;whole_sign:number;speed:number;motion:string};
type Result = {sect:{name:Sect;solar_altitude:number;rule:string};results:WheelData & {bodies:Body[];station_threshold:number;cusps:{house:number;longitude:number;position:string;whole_sign:string;sign:string;degree:number;minute:number;house_system:string}[]}; time:{utc:string;local:string;zone:string;offset_seconds:number;dst_seconds:number;tzdata:string}; astronomy:{engine:string;jd_tt:number;jd_ut1:number;node:string};warnings:string[]};
...
api('/api/house-systems').then(registry=>{
 const select=el<HTMLSelectElement>('house-system');select.replaceChildren();
 for(const system of registry.systems){const option=document.createElement('option');option.value=system.id;option.textContent=system.name;select.append(option);}
 select.value=registry.default;select.disabled=false;el<HTMLButtonElement>('calculate').disabled=false;
...
el('house-system').addEventListener('change',async()=>{
 if(!currentResult){revision++;el('status').textContent='Sistema elegido. Calculá la carta con los datos del formulario.';return;}
 const previous=currentResult,name=el('chart-name').textContent||'',current=++revision;
 lots.clearChart();profections.setNatal(null);
 currentSvg='';el('results').hidden=true;el('error').hidden=true;el('empty').hidden=true;el('status').textContent='Actualizando casas; se conservan las posiciones planetarias…';
 try{const updated=await api('/api/houses',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chart:previous,house_system:value('house-system')})});
  if(current!==revision)return;render(updated,name);el('status').textContent='Casas y rueda actualizadas. Posiciones planetarias conservadas.';
 }catch(error){if(current!==revision)return;el('error').textContent=(error as Error).message;el('error').hidden=false;el('status').textContent='No hay resultados válidos para el sistema solicitado. Seleccioná otro sistema.';}
});
...
el('place').addEventListener('input',()=>{searchVersion++;clearTimeout(searchTimer);el('place-results').replaceChildren();set('latitude','');set('longitude','');set('timezone','');el('place-status').textContent=value('place').trim().length<2?'Escribí al menos dos letras de la ciudad.':'Buscando localidades…';if(value('place').trim().length>=2)searchTimer=setTimeout(searchPlaces,500);});
el('place').addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();searchPlaces();}});
...
async function searchPlaces(){
 clearTimeout(searchTimer);if(value('place').trim().length<2){el('place-status').textContent='Escribí al menos dos letras de la ciudad.';return;}
 const version=++searchVersion;el('place-results').replaceChildren();el('place-status').textContent='Buscando localidades…';
 ...
 }catch(error){if(version===searchVersion)el('place-status').textContent=(error as Error).message;}
}
...
function render(data:Result,name:string){el('bodies').replaceChildren();el('cusps').replaceChildren();el('angles').replaceChildren();el('chart-name').textContent=name.trim()||'Carta sin nombre';
 currentResult=data;
 set('house-system',data.results.house_system.id);el('active-system').textContent=`Sistema activo: ${data.results.house_system.name}`;el('house-column').textContent=`Casa · ${data.results.house_system.name}`;
 ...
 for(const b of data.results.bodies)row('bodies',[b.name,b.position,b.house,b.whole_sign,b.speed.toFixed(6),b.motion],b.motion==='Retrógrado');
 for(const c of data.results.cusps)row('cusps',[c.house,c.sign,`${c.degree}°`,`${String(c.minute).padStart(2,'0')}′`,`${c.longitude.toFixed(6)}°`,data.results.house_system.name,c.whole_sign]);
 ...
 el('empty').hidden=true;el('results').hidden=false;lots.setChart(data);updateWheel();
}
function updateWheel(){
 if(!currentResult)return;
 const invalid=Array.from(el('orb-inputs').querySelectorAll('input')).some(input=>!input.checkValidity());
 el<HTMLButtonElement>('download-svg').disabled=invalid;el<HTMLButtonElement>('download-png').disabled=invalid;
 if(invalid){currentSvg='';el('wheel').replaceChildren();el('aspect-list').replaceChildren();el('wheel-message').textContent='Revisá los orbes: deben ser números entre 0° y 30°, sin dejarlos vacíos. No se exportan resultados con orbes inválidos.';return;}
...
for(const planet of traditionalPlanets){
 ...
 label.textContent=planet;input.type='number';input.min='0';input.max='30';input.step='any';input.required=true;input.value=String(orbs[planet]);input.dataset.planet=planet;input.setAttribute('aria-label',`Orbe ${planet} en grados`);
 input.addEventListener('input',()=>{if(input.checkValidity())orbs[planet]=Number(input.value);updateWheel();});
 label.append(input);el('orb-inputs').append(label);
}
el('reset-orbs').addEventListener('click',()=>{orbs=resetOrbs();for(const input of Array.from(el('orb-inputs').querySelectorAll('input')))input.value=String(orbs[input.dataset.planet!]);updateWheel();});
for(const [id,key] of [['show-terms','terms'],['show-aspects','aspects'],['show-degrees','degrees'],['show-cusps','cusps']] as const){el<HTMLInputElement>(id).addEventListener('change',()=>{wheelOptions[key]=el<HTMLInputElement>(id).checked;updateWheel();});}
...
el('download-svg').addEventListener('click',()=>{if(currentSvg){downloadSvg(currentSvg);el('wheel-message').textContent='SVG preparado con los glifos convertidos a trazados.';}});
el('download-png').addEventListener('click',async()=>{if(!currentSvg)return;const button=el<HTMLButtonElement>('download-png');button.disabled=true;const snapshot=currentSvg;
 try{downloadBlob(await pngBlob(snapshot),'dama-del-tiempo-3300.png');el('wheel-message').textContent='PNG de 3300 × 3300 píxeles preparado.';}catch(error){el('wheel-message').textContent=(error as Error).message;}finally{button.disabled=false;}
});
form.addEventListener('submit',async(event)=>{event.preventDefault();const current=++revision;currentResult=null;currentSvg='';lots.clearChart();profections.setNatal(null);const name=value('name');const button=el<HTMLButtonElement>('calculate');button.disabled=true;el('error').hidden=true;el('results').hidden=true;el('status').textContent='Calculando con Swiss Ephemeris…';
 try{const result=await api('/api/chart',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({date:value('date'),time:value('time'),latitude:Number(value('latitude')),longitude:Number(value('longitude')),timezone:value('timezone').trim(),fold:value('fold')===''?null:Number(value('fold')),node:value('node'),station_threshold:Number(value('threshold')),house_system:value('house-system')})});if(current!==revision)return;render(result,name);el('status').textContent='Cálculo completado.';
...
function inspectTerm(event:Event){const target=(event.target as Element).closest('[data-term]');if(!target)return;const [sign,index]=target.getAttribute('data-term')!.split('-').map(Number);el('term-info').textContent=termLabel(sign,index);}
for(const event of ['click','pointerover','focusin'])el('wheel').addEventListener(event,inspectTerm);
el('wheel').addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();inspectTerm(event);}});

el('aspect-order').addEventListener('change',updateWheel);
el('download-aspects').addEventListener('click',()=>{if(!currentResult||!currentSvg)return;const rows=compactAspects(evaluateAspects(currentResult.results.bodies,orbs),currentResult.results.bodies,value('aspect-order') as AspectOrder);downloadBlob(new Blob([compactCsv(rows)],{type:'text/csv;charset=utf-8'}),'dama-del-tiempo-aspectos.csv');});
