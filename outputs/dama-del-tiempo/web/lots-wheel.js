...
import { point, overlaps, outlinedText } from './wheel.js';
import { catalog } from './lots.js';
export function layoutLots(values, asc) {
    const placed = [];
    for (const [index, v] of values.entries()) {
        if (!v.ok || v.longitude === undefined)
            continue;
        let found = false;
        for (let ring = 0; ring < 10 && !found; ring++)
            for (let step = 0; step < 180 && !found; step++)
                for (const offset of step ? [step * 2, -step * 2] : [0]) {
                    ...
                    if (placed.every(b => !overlaps(p, b, 3))) {
                        placed.push(p);
                        found = true;
                        break;
                    }
        ...
        if (!found)
            throw Error('No hay espacio para las etiquetas de lotes seleccionadas.');
    }
    return placed;
}
const esc = (s) => s.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;').replaceAll('"', '&quot;');
export function appendLots(svg, values, asc, print = false) {
    const valid = values.filter(v => v.ok);
    if (!valid.length)
        return svg;
    ...
    for (const p of placed) {
        const anchor = point(p.longitude, asc, 610), r = catalog.find(r => r.id === p.id), v = valid.find(v => v.id === p.id);
    ...
    let y = legendTop;
    for (const p of placed) {
        const r = catalog.find(r => r.id === p.id), v = valid.find(v => v.id === p.id);
        const text = `L${p.index} · ${r?.nameSpanish || p.id} · ${v.position}`, words = text.split(' '), lines = [];
        let line = '';
        ...
        for (const l of lines) {
            layer += outlinedText(l, 550, y, 15, ink);
            y += 23;
        }
    }
    layer += '</g>';
