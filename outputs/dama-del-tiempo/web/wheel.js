...
export const defaultOptions = { aspects: true, degrees: true, cusps: true, print: false, terms: true };
export const SIZE = 1100, CENTER = 550;
export const signs = ['♈', '♉', '♊', '♋', '♌', '♍', '♎', '♏', '♐', '♑', '♒', '♓'];
export const bodyGlyphs = { 'Sol': '☉', 'Luna': '☽', 'Mercurio': '☿', 'Venus': '♀', 'Marte': '♂', 'Júpiter': '♃', 'Saturno': '♄', 'Nodo Norte': '☊', 'Nodo Sur': '☋' };
export const normalize = (value) => (value % 360 + 360) % 360;
export function longitudeToAngle(longitude, ascendant) { return (180 - (longitude - ascendant)) * Math.PI / 180; }
export function point(longitude, ascendant, radius) {
    const angle = longitudeToAngle(longitude, ascendant);
    return { x: CENTER + radius * Math.cos(angle), y: CENTER + radius * Math.sin(angle) };
}
export function houseMidpoint(start, end) { return normalize(start + normalize(end - start) / 2); }
export function overlaps(a, b, padding = 5) { return Math.abs(a.x - b.x) < (a.width + b.width) / 2 + padding && Math.abs(a.y - b.y) < (a.height + b.height) / 2 + padding; }
export function layoutBodies(bodies, asc) {
    const placed = [];
    ...
    for (const body of [...bodies].sort((a, b) => normalize(a.longitude) - normalize(b.longitude) || a.name.localeCompare(b.name))) {
        let chosen;
        const offsets = [0];
        for (let offset = 3; offset <= 180; offset += 3)
            offsets.push(offset, -offset);
        for (const offset of offsets) {
            for (const radius of [304, 236]) {
                const p = point(body.longitude + offset, asc, radius);
                const candidate = { ...p, width: 90, height: 72, name: body.name, longitude: body.longitude, labelLongitude: normalize(body.longitude + offset), radius, anchor: point(body.longitude, asc, 400) };
        ...
        if (!chosen)
            throw Error('No se pudieron distribuir los glifos sin superposición.');
        placed.push(chosen);
    }
    return placed;
}
...
function textWidth(text, size) { return Array.from(text).reduce((sum, char) => sum + glyphs[char].advance * size / glyphs[char].units, 0); }
export function layoutCuspLabels(cusps, asc, mc, showCusps = true) {
    const placed = [];
    ...
    const same = (a, b) => Math.abs(normalize(a - b + 180) - 180) < 1e-7;
    ...
    for (const axis of axes)
        if (!items.some(item => same(item.longitude, axis.longitude)))
            items.push({ house: 0, longitude: axis.longitude, position: '', prefix: axis.name });
    for (const cusp of items) {
        const text = cusp.prefix + ' · ' + degrees(cusp.longitude), width = textWidth(text, 16) + 6, height = 23;
        let found = false;
        for (let shift = 0; shift <= 180 && !found; shift += 2)
            for (const offset of shift ? [shift, -shift] : [0]) {
                const labelLongitude = cusp.longitude + offset;
                let rotation = normalize(longitudeToAngle(labelLongitude, asc) * 180 / Math.PI + 90);
                ...
                if (placed.every(other => !overlaps(candidate, other, 4))) {
        ...
        if (!found)
            throw Error('No se pudieron distribuir las etiquetas de cúspides.');
    }
    return placed;
}
export function outlinedText(text, x, y, size, color, rotation = 0) {
    // Cada glifo es un trazado: SVG/PNG no dependen de fonts, CSS o red externos.
    const letters = Array.from(text).map(char => { const glyph = glyphs[char]; if (!glyph)
        throw Error(`Glifo no disponible: ${char}`); return glyph; });
...
const line = (a, b, stroke, width = 1, attrs = '') => `<line x1="${f(a.x)}" y1="${f(a.y)}" x2="${f(b.x)}" y2="${f(b.y)}" stroke="${stroke}" stroke-width="${width}" ${attrs}/>`;
const circle = (radius, stroke, width = 1, fill = 'none') => `<circle cx="550" cy="550" r="${radius}" stroke="${stroke}" stroke-width="${width}" fill="${fill}"/>`;
function degrees(longitude) { const minutes = Math.floor(normalize(longitude) * 60 + .5) % 21600; return `${String(Math.floor(minutes % 1800 / 60)).padStart(2, '0')}°${String(minutes % 60).padStart(2, '0')}′`; }
function roundedSign(longitude) { return signs[Math.floor((Math.floor(normalize(longitude) * 60 + .5) % 21600) / 1800)]; }
const aspectStyles = { Conjunción: { color: '#7e6477', dash: '' }, Sextil: { color: '#16212f', dash: '3 5' }, Cuadratura: { color: '#7e6477', dash: '10 4' }, Trígono: { color: '#16212f', dash: '' }, Oposición: { color: '#2e2046', dash: '12 4 2 4' } };
export function renderWheel(data, options, aspects, title = 'Dama del Tiempo') {
    const asc = data.angles.find(a => a.name === 'Ascendente')?.longitude, mc = data.angles.find(a => a.name === 'Medio Cielo')?.longitude;
    if (asc === undefined || mc === undefined || data.cusps.length !== 12 || !Number.isFinite(asc) || !Number.isFinite(mc))
        throw Error('Faltan ángulos o cúspides del resultado calculado.');
    if ([...data.bodies, ...data.cusps].some(b => !Number.isFinite(b.longitude)))
        throw Error('Longitud no válida en la carta.');
    ...
    if (!system?.id || !system.name)
        throw Error('Falta identificar el sistema de casas del resultado.');
    ...
    for (let sign = 0; sign < 12; sign++) {
        ...
        const p = point(sign * 30 + 15, asc, 457);
        svg += outlinedText(signs[sign], p.x, p.y, 35, sign % 2 ? violet : ink);
    }
    for (let degree = 0; degree < 360; degree++) {
        ...
        if (tier === 10) {
            const p = point(degree + 1.6, asc, 414);
            svg += outlinedText(String(degree % 30), p.x, p.y, 11, ink);
        }
    }
    if (options.cusps) {
        const numberBoxes = [];
        for (let i = 0; i < 12; i++) {
            ...
            for (const r of [373, 389, 359]) {
                const test = { ...point(mid, asc, r), width: 19, height: 17 };
                if (numberBoxes.every(box => !overlaps(test, box, 1))) {
                    p = test;
                    break;
                }
            }
            numberBoxes.push({ ...p, width: 19, height: 17 });
            svg += outlinedText(String(cusp.house), p.x, p.y, 16, ink);
    ...
    for (const [name, lon] of [['ASC', asc], ['DSC', normalize(asc + 180)], ['MC', mc], ['IC', normalize(mc + 180)]]) {
        svg += line(point(lon, asc, 184), point(lon, asc, 487), violet, 2, `data-axis="${name}" data-longitude="${lon}"`);
    }
    if (options.aspects)
        for (const aspect of aspects) {
            const a = data.bodies.find(b => b.name === aspect.from), b = data.bodies.find(b => b.name === aspect.to);
            if (!a || !b)
                continue;
            const style = aspectStyles[aspect.name];
            if (!style)
                continue;
            ...
            if (aspect.angle === 0) {
                ...
                svg += line(p, q, color, 1.4, `stroke-dasharray="${style.dash}"`);
            svg += '</g>';
        }
    ...
    for (const placement of placed) {
        ...
        svg += `<g data-body-anchor="${esc(placement.name)}" data-longitude="${placement.longitude}">` + line(p, point(placement.longitude, asc, 346), muted, .9) + line(point(placement.longitude, asc, 346), placement, muted, .9) + `<circle cx="${f(p.x)}" cy="${f(p.y)}" r="2.5" fill="${ink}"/></g>`;
    }
    for (const p of placed) {
        ...
        if (body.motion === 'Retrógrado')
            svg += outlinedText('℞', p.x + 23, p.y - 10, 17, ink);
        if (options.degrees) {
            svg += outlinedText(degrees(body.longitude), p.x - 5, p.y + 19, 17, ink) + outlinedText(roundedSign(body.longitude), p.x + 34, p.y + 19, 16, muted);
        }
        svg += '</g>';
    }
    if (options.terms)
        svg += renderTerms(asc, ink, muted, bg);
    return svg + '</svg>';
}
export function termLabel(sign, index) { const row = terms.intervals[sign], t = row.segments[index]; return `${row.sign}: [${t.start}°, ${t.end}°), ${t.ruler}. Sistema: ${terms.system}. Fuente: ${terms.source}. ${terms.reference}`; }
export function termGeometry(asc) { return terms.intervals.flatMap((row, sign) => row.segments.map((t, index) => ({ sign, index, ...t, startLongitude: sign * 30 + t.start, endLongitude: sign * 30 + t.end, label: point(sign * 30 + (t.start + t.end) / 2, asc, 572), boundary: point(sign * 30 + t.start, asc, 595) }))); }
function renderTerms(asc, ink, muted, bg) {
    ...
    for (const t of termGeometry(asc)) {
        ...
        let rotation = normalize(longitudeToAngle(t.startLongitude, asc) * 180 / Math.PI + 90);
        if (rotation > 90 && rotation < 270)
            rotation += 180;
...
export function downloadSvg(svg) { downloadBlob(new Blob([svg], { type: 'image/svg+xml;charset=utf-8' }), 'dama-del-tiempo.svg'); }
export function downloadBlob(blob, filename) { const url = URL.createObjectURL(blob), link = document.createElement('a'); link.href = url; link.download = filename; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); }
export async function pngBlob(svg, size = 3300) {
    ...
    try {
        ...
        if (!ctx)
            throw Error('No se pudo preparar la imagen.');
        ctx.drawImage(image, 0, 0, size, size);
        return await new Promise((resolve, reject) => canvas.toBlob(blob => blob ? resolve(blob) : reject(Error('No se pudo generar PNG.')), 'image/png'));
    }
    finally {
