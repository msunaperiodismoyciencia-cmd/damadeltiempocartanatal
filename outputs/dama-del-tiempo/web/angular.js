...
function roundedUnits(value, units) {
    if (!Number.isFinite(value))
        throw Error('Distancia angular no finita.');
...
export function angularMinutes(value) {
    const total = roundedUnits(value, 60);
    return `${Math.floor(total / 60)}°${String(total % 60).padStart(2, '0')}′`;
}
export function angularSeconds(value) {
...
export function aspectMovement(a, b, angle) {
    if (![a.longitude, b.longitude, angle].every(Number.isFinite) || angle < 0 || angle > 180)
        return 'Indeterminado';
    const delta = ((b.longitude - a.longitude) % 360 + 540) % 360 - 180;
    const distance = Math.abs(delta), error = distance - angle;
    if (Math.abs(error) <= PERFECTION_TOLERANCE)
        return 'Partil';
    if (!Number.isFinite(a.speed) || !Number.isFinite(b.speed))
        return 'Indeterminado';
    const relative = b.speed - a.speed;
    if (Math.abs(relative) <= RELATIVE_SPEED_TOLERANCE)
        return 'Indeterminado';
    ...
    if (distance <= PERFECTION_TOLERANCE || 180 - distance <= PERFECTION_TOLERANCE)
        return 'Indeterminado';
